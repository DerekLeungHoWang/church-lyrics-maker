...
export default function Home({ locale }) {
  ...
  const initializeState = () => JSON.parse(localStorage.getItem("cart")) || [];
      ...
      getSongList().then((songList) => {
        let target = songList.data.filter(
          (d) =>
            ...
            lyricsObject.title.trim().toLowerCase()
        );
        let isExist =
          cart.length > 0 &&
          cart.some((obj) => {
  ...
  const handleDeleteFromTable = (e) => {
    ...
    axios.delete(`${API_BASE_URL}/lyrics/${deleteDialog.id}`).then((res) => {
  ...
  const getSongList = () => {
    ...
    return axios.get(`${API_BASE_URL}/lyrics/getAll`).then((res) => {
  ...
  const onDragEnd = ({ destination, source, draggableId }) => {
    ...
    const newCart = reorder(cart, source.index, destination.index);
